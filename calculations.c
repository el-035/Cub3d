...
void	calculate_dir(t_game *game, t_input *input)
{
	init_step_pos(game);
	dist_calc(game);
	while (game->ray->y >= 0 && game->ray->y < input->line_count
		&& game->ray->x >= 0 && game->ray->x < input->line_length)
	{
	...
	game->ray->distance = game->ray->distance / (game->ray->ray_dir_x
			* game->dir_x + game->ray->ray_dir_y * game->dir_y);
}
